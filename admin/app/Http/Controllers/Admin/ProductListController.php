...
class ProductListController extends Controller
{
    public function ProductListByRemark(Request $request){
        ...
        $productlist = ProductList::where('remark', $remark)->limit(12)->get();
        return $productlist;
    }
    ...
    public function ProductBySearch(Request $request){
        $key = $request->key;
        $productlist = ProductList::where('title', 'LIKE', "%{}%")->orWhere('brand', 'LIKE', "%{$key}%")->get();
        return $productlist;
    }
    ...
    public function UpdateProduct(Request $request)
    {
        ...
        //IMAGE Thubnail Update
        if ($request->file('image')) {
            $old_image = str_replace("http://127.0.0.1:8000/", '', $product->image);
        ...
        if ($request->file('image_one')) {
            $old_image = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_one);
        ...
        if ($request->file('image_two')) {
            $old_image = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_two);
        ...
        if ($request->file('image_three')) {
            $old_image = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_three);
        ...
        //IMAGE FOUR Update
        if ($request->file('image_one')) {
            $old_image = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_four);

            if (file_exists(public_path($old_image))) {
                unlink($old_image);
            }

            $image4 = $request->file('image_one');
            $name_gen4 = hexdec(uniqid()) . '.' . $image4->getClientOriginalExtension();
            Image::make($image4)->resize(711, 960)->save('upload/productdetails/' . $name_gen4);
            $save_url4 = 'http://127.0.0.1:8000/upload/productdetails/' . $name_gen4;
    ...
    public function DeleteProduct($id)
    {
        ...
        $image = str_replace("http://127.0.0.1:8000/", '', $product->image);
        if (file_exists(public_path($image)) &&  $image != null) {
            unlink($image);
        }

        $image_one = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_one);
        if (file_exists(public_path($image_one))) {
            unlink($image_one);
        }

        $image_two = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_two);
        if (file_exists(public_path($image_two))) {
            unlink($image_two);
        }

        $image_three = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_three);
        if (file_exists(public_path($image_three))) {
            unlink($image_three);
        }

        $image_four = str_replace("http://127.0.0.1:8000/", '', $productDetails->image_four);
        if (file_exists(public_path($image_four))) {
            unlink($image_four);
        }
